...
.uuid-wrap{display:flex;align-items:center;justify-content:flex-end;gap:8px;min-width:0;max-width:72%;position:relative}
.uuid-wrap .val{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.copy-btn{width:30px;height:30px;border:1px solid var(--pbtn-b);border-radius:8px;background:var(--pbtn);color:var(--pbtn-hc);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;transition:background .15s,border-color .15s,color .15s}
.copy-btn:hover{border-color:var(--pbtn-hb);background:var(--net-h);color:#8ab4f8}
.copy-btn svg{width:15px;height:15px;stroke:currentColor;fill:none;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round}
.copy-hint{position:absolute;right:0;top:-24px;background:#2a3f6e;color:#8ab4f8;border:1px solid #3a5fa8;border-radius:7px;padding:3px 7px;font-size:11px;font-weight:700;opacity:0;transform:translateY(3px);pointer-events:none;transition:opacity .16s,transform .16s;white-space:nowrap}
...
<div class="card">
  ...
  <div class="row"><span class="lbl">广播 UUID</span><span class="uuid-wrap"><span id="ble_uuid_show" class="val">--</span><button class="copy-btn" onclick="copyBleUuid()" title="复制广播 UUID" aria-label="复制广播 UUID"><svg viewBox="0 0 24 24"><rect x="9" y="9" width="11" height="11" rx="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg></button><span id="ble_copy_hint" class="copy-hint">已复制</span></span></div>
  <div class="row"><span class="lbl">周期 / 时长</span><span id="ble_window_show" class="val">--</span></div>
...
<div class="card">
  ...
  <div class="pad" style="border-top:1px solid var(--sep)">
    ...
    <div class="tog" style="padding:10px 0 0;border-bottom:none">
      <span class="tlbl">启动时使用当前时序</span>
      <label class="sw"><input type="checkbox" id="ble_use_saved_timing"><span class="sl"></span></label>
    </div>
...
function switchTab(name) {
  ...
  if (name === 'wifi') {
    setTimeout(refreshSystemPage, _wifiLoadedOnce ? 80 : 0);
    _wifiLoadedOnce = true;
  }
}
...
function renderBleTiming() {
  if (!_bleCountdown) return;
  const elapsed = Date.now() - _bleCountdown.t0;
  const remain = Math.max(0, _bleCountdown.remaining - elapsed);
  const label = _bleCountdown.advertising ? '剩余 ' : '下次周期 ';
  txt('ble_window_show', _bleCountdown.window + 'ms / ' + _bleCountdown.duration + 'ms · ' + label + fmtMs(remain));
}
...
async function loadBle() {
  try {
    ...
    _bleCountdown = {
      ...
      remaining: d.advertising ? d.adv_remaining_ms : d.cycle_remaining_ms,
      window: d.window_ms,
    ...
    const useTiming = g('ble_use_saved_timing'); if (useTiming) useTiming.checked = !!d.use_saved_timing;
  } catch(e) {
...
async function copyBleUuid() {
  const text = (g('ble_uuid_show').textContent || '').trim();
  if (!text || text === '--') return;
  try {
    if (!await copyText(text)) throw new Error('copy failed');
    const hint = g('ble_copy_hint');
...
async function saveBle() {
  ...
  const p = new URLSearchParams({
    ...
    use_saved_timing: g('ble_use_saved_timing').checked ? '1' : '0'
  });
  try {
...
setInterval(() => { if (_activeTab === 'wifi') loadAp(); }, 15000);
setInterval(() => { if (_activeTab === 'wifi') loadSta(); }, 6000);
setInterval(() => { if (_activeTab === 'wifi') loadBle(); }, 2000);
setInterval(() => { if (_activeTab === 'wifi') renderBleTiming(); }, 200);
setInterval(() => { if (_activeTab === 'debug') dbgPoll(); }, 1500);
</script>
</body>
</html>)HTML";
