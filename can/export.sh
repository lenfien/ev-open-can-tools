...
cat dnsmasq.log | grep query | grep 172.16.0.123 | awk '{print $6}' | sort -u | grep 'tesla'| awk '{print "server=/" $0 "/223.5.5.5"}' > tesla_domain/tesla_domains_$(date +"%Y%m%d%H%M%S").txt
